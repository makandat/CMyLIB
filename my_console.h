...
void my_on_badacess(func1_t handler);  // メモリ不正アクセスがあったときのハンドラを登録する。
void my_on_abort(func1_t handler);  // abort() がコールされたときのハンドラを登録する。
void my_on_siguser(func1_t handler);  // ユーザ定義シグナルが発せられたときのハンドラを登録する。
void my_on_jump(jmp_buf* saved, func0_t handler);  // longjmp を行ったときのハンドラを登録する。
...
MY_HEAP const char* my_input(const char* message);  // メッセージを stdout に表示してから stdin から１行入力する。
